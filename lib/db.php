...
function set_fs_contactdata($fsid, $phone, $address, $email, $satzung, $www, $iban){
	$query = "UPDATE ".DB_PREF."fachschaften SET telefon='$phone', adresse='$address', email='$email', satzung='$satzung', www='$www', iban='$iban' WHERE id = $fsid;";
	$result = mysql_query($query);
	if($result){
		return true;
	} else {
